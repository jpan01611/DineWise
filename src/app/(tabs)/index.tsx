...
export default function HomeScreen() {
  ...
  const { configuredPlan, configuredSummary, setupFlow } = useLocalSearchParams<{
  ...
  React.useEffect(() => {
    if (!configuredPlan || typeof configuredPlan !== 'string') return;
    const cleanPlan = configuredPlan.trim();
    if (!cleanPlan) return;

    setConfiguredDiningSession(cleanPlan);
    setDiningSessionConfigured(true);
    if (configuredSummary && typeof configuredSummary === 'string' && configuredSummary.trim()) {
      setDiningSystemSummary(configuredSummary.trim());
    }

    if (setupFlow === 'onboarding') {
      setHasOnboarded(false);
      setOnboardingStep(4);
    } else {
      setHasOnboarded(true);
    }
  }, [configuredPlan, configuredSummary, setupFlow]);
  ...
  const openMealPlanSetup = (flow: 'dashboard' | 'onboarding' = 'dashboard') => {
    const plans = diningSystems;
    router.push({
      pathname: '/meal-plan-setup',
      params: {
        school,
        plans: JSON.stringify(plans),
        backgroundHex: universityTheme.background.replace('#', ''),
        backgroundElementHex: universityTheme.backgroundElement.replace('#', ''),
        setupFlow: flow,
      },
    });
  };
          ...
          <View style={[styles.stepContainer, { backgroundColor: cardBg }]}> 
            {currentStep.key === 'meal-plan' ? (
              <View style={styles.sessionSetupRow}>
                <Text style={{ color: contrastColor(cardBg), fontWeight: '600', fontSize: 14 }}>
                  Select your meal plan option
                </Text>
                {!diningSystems.length ? (
                  <Text style={{ color: contrastColor(cardBg) + 'cc', fontSize: 13 }}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                  style={[styles.dropdownTrigger, { borderColor: contrastColor(cardBg) + '35' }]}>
                  <Text style={{ color: contrastColor(cardBg), fontWeight: configuredDiningSession ? '700' : '500' }}>
                    {configuredDiningSession || 'Select an option'}
                  </Text>
                  <Text style={{ color: contrastColor(cardBg), fontWeight: '700' }}>
                    {mealPlanDropdownOpen ? '▲' : '▼'}
                  </Text>
                </TouchableOpacity>

                {mealPlanDropdownOpen ? (
                  <View style={[styles.onboardingOptionsList, { borderColor: contrastColor(cardBg) + '35' }]}>
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          <View style={styles.onboardingOptionRow}>
                            <Text style={{ color: contrastColor(cardBg), fontWeight: isSelected ? '700' : '500' }}>
                              {option}
                            </Text>
                            <Text style={{ color: contrastColor(cardBg), fontWeight: '700' }}>
                              {isSelected ? '✓' : ''}
            ...
            <View style={{ flexDirection: 'row', gap: Spacing.two, marginTop: Spacing.two }}>
              ...
              <TouchableOpacity
                onPress={onStepNext}
                disabled={themeLoading || (currentStep.key === 'meal-plan' && !diningSessionConfigured)}
                style={{ flex: 2 }}>
                <View style={{
                  backgroundColor: pageBg,
                  opacity: (themeLoading || (currentStep.key === 'meal-plan' && !diningSessionConfigured)) ? 0.5 : 1,
                  paddingVertical: 12,
                  borderRadius: 8,
                  alignItems: 'center',
                }}>
        ...
        >
          <View style={[styles.mainContent, Platform.OS === 'web' ? styles.mainContentWeb : styles.mainContentMobile]}>
            ...
            <ThemedView type="backgroundElement" style={[styles.sessionPanel, { backgroundColor: universityTheme.backgroundElement }]}> 
              <ThemedText type="smallBold" style={{ color: contrastColor(universityTheme.backgroundElement) }}>Meal plan</ThemedText>
              {!diningSessionConfigured ? (
                <View style={styles.sessionSetupRow}>
                  <ThemedButton title="Configure meal plan" onPress={() => openMealPlanSetup('dashboard')} theme={universityTheme} />
                </View>
              ) : (
                <View style={styles.sessionSetupRow}>
                  <ThemedText type="smallBold" style={{ color: contrastColor(universityTheme.backgroundElement) }}>
                    Configured: {configuredDiningSession}
                  </ThemedText>
                  {diningSystemSummary ? (
                    <ThemedText type="small" style={{ color: contrastColor(universityTheme.backgroundElement) }}>
                      {diningSystemSummary}
                    </ThemedText>
                  ) : null}
                  <ThemedButton title="Edit meal plan" onPress={() => openMealPlanSetup('dashboard')} theme={universityTheme} />
                </View>
              )}
            </ThemedView>
